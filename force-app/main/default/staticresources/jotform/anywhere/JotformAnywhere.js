...
if (!window.console) console = {
  ...
  function removeElementByQuery(eClass) {
      return (EObj = document.querySelector(eClass)) ? EObj.parentNode.removeChild(EObj) : false
  }
  ...
  var _jotformURL = "https://www.jotform.com";
   //var _jotformURL = "https://cem.jotform.pro";
  
  function addJFXDListener(url) {
  ...
  var JotformAnywhere = (function() {
    ...
    root.useEnterprise = function(enterpriseSubDomain) {
        if (!enterpriseSubDomain) {
          console.log('Enterprise sub domain required to use JotForm Anyhwere with JotForm Enterprise');
          return;
        }
        _jotformURL = 'https://' + enterpriseSubDomain + '.jotform.com';
        addJFXDListener(_jotformURL);
        console.log('now using JotForm Anyhwere with JotForm Enterprise', _jotformURL);
    }
    ...
    root.launchFormBuilder = root.launchBuilder = function(options) {
      ...
      var returnIframe = (!('returnIframe' in options)) ? false : options.returnIframe; //if true Jotform iframe is not rendered by SDK instead returns builded iframe DOM object
      var openInModal = (!('openInModal' in options)) ? true : options.openInModal; //if true SDK opens buider frame in modalBox, notice that if true insertTo option has no effect
      var builderMaskColor = options.builderMaskColor || "#000000";
      var themeID = options.themeID || 'false';
      var fullscreen = ('fullscreen' in options && options.fullscreen) ? true : false;
  
      var frameWidth = options.width ? options.width : builderFrameWidth;
      var frameHeight = options.height || builderFrameHeight;
      ...
      var insertForm = true;
      if (options.insertForm !== undefined) {
        insertForm = options.insertForm;
      }
      ...
      if(!options.editMode) {
            frameSrc = _jotformURL + "/api/anywhere/embed_builder.php";
        } else {
            frameSrc = _jotformURL + "/build/" + formID;
        }
      ...
      // build frame src
      frameSrc += "?ref=" + encodeURIComponent(ref) +
        "&fbbc=" + encodeURIComponent(root.formBuilderBarColor) +
        "&pbc=" + encodeURIComponent(root.primaryButtonColor) +
        "&pbhc=" + encodeURIComponent(root.primaryButtonHoverColor) +
        "&inf=" + encodeURIComponent(insertForm + "") +
        "&rle=" + encodeURIComponent(remember + "") +
        "&tid=" + encodeURIComponent(themeID);
  
      if (fullscreen) {
        frameSrc += "&fs=" + encodeURIComponent(fullscreen);
      }
      ...
      frame.style.width = frameWidth.toString().indexOf('%') > 0 ? frameWidth : frameWidth + "px";
      ...
      if (!returnIframe) {
        if (!openInModal) {
          ...
          // give addition padding on the iframe
          frame.style.padding = '10px';
  
          var container = document.body;
  
          if (insertTo !== undefined) {
            container = document.querySelector(insertTo);
          }
          ...
          if (insertTo === undefined) {
            ...
            iframeContainer.style.top = (innerHeight - frameHeight) / 4 + "px";
            iframeContainer.style.left = (innerWidth - frameWidth) / 2 + "px";
          }
          ...
          closeButton.onclick = function() {
            if(options.editMode) {
              var mainFrameElement = document.getElementById(formID);
              mainFrameElement.src = _jotformURL + '/' + formID + '?noToolBar';
            }
    ...
    root.removeBuilder = function() {
      if (document.getElementById("jotform_iframe_wrapper")) {
        removeElement("jotform_iframe_wrapper");
      }
  
      if (document.getElementById("jotform_builder_mask")) {
        removeElement("jotform_builder_mask");
      }
  
      if (document.getElementById("jotform_builder_frame")) {
        removeElement("jotform_builder_frame");
      }
    };
  
    root.insertForm = function(options) {
  
      //Remove builder frame
          removeElement("jotform_iframe_wrapper");
  
          //Removing prevous containers
          removeElementByQuery(".jotform-form-container");
          removeElementByQuery(".form-delete-button");
          removeElementByQuery(".form-edit-button");
      ...
      if (options.formEmbedUrl === undefined) {
        ...
        embedURL = options.formEmbedUrl.replace('http://', 'https://');
      }
      ...
      if (editMode) {
        ...
        if (putEditButton) {
          ...
          editButton.onclick = function() {
            root.launchBuilder({
              insertForm: root.builderOptions.insertForm,
              formID: formID,
              openInModal: true,
              // insertTo: "#content",
              returnIframe: false,
        ...
        if (putDeleteButton) {
          ...
          deleteButton.onclick = function() {
            ...
            root.publish("jotform.formDeleted", {
              formID: formID
            });
  
            // Deleting cookies to prevent display of form while revisiting the page
            del_cookie('form-id');
            del_cookie('last_form');
      ...
      formScript.src = jsFormEmbedUrl + "?noToolBar&appendTo=jotform_form_container_" + formID;
    ...
    root.subscribe("jotform.formUpdated", function(response) {
      ...
      if (root.builderOptions.insertForm === false) {
        return;
      }
